...
exports.run = async (client, message, args) => {
    ...
    User.findOne({ _id: USER.id }, async function (err, user) {
        ...
        if(!user.addBot.haveBot){
            return message.channel.send(`${message.author}, este membro não possui um Bot no servidor.`)
        } else {
            await User.findOneAndUpdate({_id: USER.id}, {$set: { 'addBot.haveSoli': false, 'addBot.haveBot': false, 'addBot.acceptBy': "null", 'addBot.acceptIn': 0 }})
