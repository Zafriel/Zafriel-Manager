  ...
  Guild.findOne({ _id: message.guild.id }, async function (err, server) {
    ...
    if (!USER)
      return message.channel.send(
        `${message.author}, você deve mencionar/inserir o ID do membro que deseja setar como Staff.`
      );
    if (server.staff.some((x) => x === USER.id)) {
      return message.channel.send(
        `${message.author}, o usuário já está setado como Staff.`
      );
    } else {
      message.channel.send(
        `${message.author}, ok, agora o usuário **${USER.user.tag}** tem permissões de Staff.`
      );
